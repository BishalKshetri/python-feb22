...
# # 1. Check status (VERY IMPORTANT - use often)
# git status
...
# # NOTE (Windows issue):
# # If filename has special characters like &
# git add "file&name.py"

# # 3. Commit changes (MANDATORY before push)
# git commit -m "your message"

# # Shortcut (only for already tracked files)
...
# # Fetch (see updates without merging)
# git fetch
...
# # If you see ../folder in git status:
# # means repo is initialized in wrong location

# git rev-parse --show-toplevel   # check repo root

# # Fix by removing .git (be careful)
# rmdir /s /q .git   # Windows

# # Then reinitialize in correct folder
# git init
